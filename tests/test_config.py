...
class TestValidConfig:
    ...
    def test_unknown_sub_key_is_tolerated(self):
        ...
        cfg = {"site1": {"pool": "192.10.0.0/16", "bmc": "10.50.0.0/16", "future": 1}}
        r = run_startup(json.dumps(cfg))
        assert r.returncode == 0, r.stderr
        assert "unrecognised" in r.stderr

    def test_bmc_is_optional(self):
...
class TestInvalidConfig:
    ...
    def test_pool_overlapping_a_bmc_network_refuses(self):
        """The check that justifies `bmc` living in this service's config at all:
        request validation can never catch it, because containment runs first."""
        cfg = {"site1": {"pool": "10.50.0.0/16"},
               "site2": {"pool": "193.51.0.0/16", "bmc": "10.50.0.0/16"}}
        assert_refused(run_startup(json.dumps(cfg)), "BMC network")
