...
- **Network** — CIDR format & strict network address, **site pool containment** (the segment must be a subnet of its site’s configured pool, or match one of that site’s `pool-exceptions` entries exactly), subnet mask /16–/31, reserved-range rejection, overlap detection.
